...
namespace DSA.Course._04_Array.Problems
{
    ...
    //Medium        Accuracy: 35.15%        Submissions: 189K+      Points: 4
    ...
    internal static class _18_RearrangeArrayAlternately
    {
        // Time Complexity: O(N)
        // Auxiliary Space: O(1)
        ...
        {
            int maxValue = arr.Max() + 1;

            for (int i = 0, j = n - 1; i < n; i += 2, j--)
                arr[i] = arr[i] + (arr[j] % maxValue) * maxValue;
