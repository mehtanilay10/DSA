...
{
    //Lucky Numbers
    //Easy  Accuracy: 49.91%    Submissions: 50938  Points: 2
    ...
    //Constraints:
    //1 <= N <= 105
    internal static class _03_LuckyNumber
    {
        internal static void Way01()
        {
            int number = ConsoleHelper.ReadInteger();
            bool isLucky = Way01IsLucky(number);
            Console.WriteLine($"IsLucky: {isLucky}");
        }

        private static bool Way01IsLucky(int number, int counter = 2)
        {
            if (number % counter == 0)
                return false;
            ...
            return Way01IsLucky(number - (number / counter), counter + 1);
        }
    }
}
